...
class Lua(LuaImplPackage):
    ...
    with when("build_system=makefile platform=windows"):
    ...
    with when("platform=linux,darwin,cray"):
        depends_on("ncurses+termlib")
        depends_on("readline")

    def build(self, spec, prefix):
        if spec.satisfies("platform=windows %msvc"):
            self._build_msvc(spec, prefix)
        elif spec.satisfies("platform=darwin"):
            self._build_unix(spec, prefix, "macosx")
        else:
            self._build_unix(spec, prefix, "linux")
    ...
    def _build_unix(self, spec, prefix, target):
        ...
        if not spec.satisfies("platform=windows"):
            make_args.extend([
                "MYLDFLAGS=" + " ".join((spec["readline"].libs.search_flags, spec["ncurses"].libs.search_flags)),
    ...
    def install(self, spec, prefix):
        if spec.satisfies("platform=windows %msvc"):
            self._install_msvc(spec, prefix)
        else:
    ...
    def _install_unix(self, spec, prefix):
